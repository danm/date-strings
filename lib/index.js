...
var getFilteredValues = function getFilteredValues(date) {
  ...
  var months = now.getMonth() + 1 - (date.getMonth() + 1);
  ...
  if (months > 0) {
    ...
    var tempMonth = new Date(now);
    tempMonth.setMonth(tempMonth.getMonth() - months);
    var msDeducted = now.getTime() - tempMonth.getTime();
    ms -= msDeducted;
  }

  var days = Math.round(ms / msInDays);
  if (days > 0) {
    ms -= days * msInDays;
  }
  ...
  var hours = Math.round(ms / msInHours);
  if (hours > 0) {
    ms -= hours * msInHours;
  }
  var minutes = Math.round(ms / msInMins);
  if (minutes > 0) {
    ms -= minutes * msInMins;
  }
  var seconds = Math.round(ms / msInSeconds);
  if (seconds > 0) {
